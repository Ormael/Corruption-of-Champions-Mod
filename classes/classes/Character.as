...
//			if (findStatusAffect(StatusAffects.GooStuffed) >= 0) return; //No longer needed thanks to PREGNANCY_GOO_STUFFED being used as a blocking value
		...
		public function maxHP():Number
		{
			...
			if (findPerk(PerkLib.TankI) >= 0) max += Math.round(tou);
			if (findPerk(PerkLib.TankII) >= 0) max += Math.round(tou);
			if (findPerk(PerkLib.TankIII) >= 0) max += Math.round(tou);
			if (findPerk(PerkLib.TankIV) >= 0) max += Math.round(tou);
			if (findPerk(PerkLib.TankV) >= 0) max += Math.round(tou);
